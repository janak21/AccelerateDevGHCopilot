...
using Library.Infrastructure.Data; // added
...
public class ConsoleApp
{
    ...
    private readonly JsonData _jsonData; // added

    public ConsoleApp(ILoanService loanService, IPatronService patronService, IPatronRepository patronRepository, ILoanRepository loanRepository, JsonData jsonData) // updated
    {
        _patronRepository = patronRepository;
        _loanRepository = loanRepository;
        _loanService = loanService;
        _patronService = patronService;
        _jsonData = jsonData; // added
    }
    ...
    async Task<ConsoleState> SearchBooks()
    {
        ...
        // find a book item for that book
        var bookItem = _jsonData.BookItems?.FirstOrDefault(bi => bi.BookId == book.Id);
        if (bookItem == null)
        {
            Console.WriteLine($"{book.Title} is not currently in the inventory.");
            return ConsoleState.PatronDetails;
        }

        // check for an active loan (ReturnDate == null) for that book item
